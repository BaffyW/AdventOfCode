#Learning from Jonathan Paulson https://www.youtube.com/watch?v=bhNhHHEZHMI

import re
fileInput = open("passports.txt")
#fileInput = open("sample.txt")
lines = list(fileInput)
lines.append('') #Skapar en blankrad så att vi räknar med den första/sista också;
#så att man inte missar ett pass.

ans = 0 #nr of valid passports
...
for line in lines:
    ...
    if not line:
        ...
        if 'hgt' in passport:
            ...
            """ht_n = re.findall('\d+', ht)
            if ht.endswith('in'):
                if not 59 <= int(ht_n[0]) <= 193:
                    valid = False
            elif ht.endswith('cm'):
                if not 150 <= int(ht_n[0]) <= 193:
                    valid = False
            else:
                valid = False"""
        else:
